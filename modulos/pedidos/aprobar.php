...
?> </section>
    <section class="content">
        ...
        if (isset($_POST['aprobar'])) {
            //$pedidos = $Pedidos->udp_pedido_cant();
            /*$obtpedcant= $Pedidos->get_pedido_productos();
            while ($row=mysqli_fetch_array($obtpedcant)) {
                $existe=false;
                for ($i=0; $i < count($_SESSION['pedcant']); $i++) { 
                    if ($_SESSION['pedcant'][$i][0]==$row['cve']) {
                        $existe=true;
                    }
                }
                if ($existe==false) {
                    $eliprod = $Pedidos->delete_produto_ped($row['cve']);
                }
            }*/
            for ($i=0; $i < count($_SESSION['pedcant']); $i++) {
                $aprobarp = $Pedidos->update_pedido_cant($i);
                $aprped = $Pedidos->update_pedidos();
                $apralm = $Pedidos->update_almacen_stock_cant($i);
            }
        }
        ?>
